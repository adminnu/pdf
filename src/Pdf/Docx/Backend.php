...
class Backend extends Container implements BackendInterface
{
     ...
     *
     * @var \SimpleXMLElement documentXML
     */
     ...
     * @return \SimpleXMLElement
     */
    public function getHeaderXMLs()
    {
        return $this->headerXMLs;
    }

    /**
     * @return \SimpleXMLElement
     */
    public function getFooterXMLs()
    {
        return $this->footerXMLs;
    }
     ...
     * string
     */
    public function normaliseStartTag($value)
    {
    ...
    public function setValues($translate)
    {
        foreach ($this->headerXMLs as $index => $headerXML) {
            $this->headerXMLs[$index] = $this->xml(strtr($this->headerXMLs[$index]->asXml(), $translate));
        }
        $this->documentXML = $this->xml(strtr($this->documentXML->asXml(), $translate));

        foreach ($this->footerXMLs as $index => $headerXML) {
            $this->footerXMLs[$index] = $this->xml(strtr($this->footerXMLs[$index]->asXml(), $translate));
        }
    }
    ...
    {
        while ($matches = $this->searchForBlock($this->documentXML, $blockname)) {
            $this->documentXML = $this->xml(str_replace
            (
                $matches[0],
                $matches[1],
    ...
    public function cloneRow($search, $numberOfClones)
    {
        ...
        $xmlRow = mb_substr($xml, $rowStart, $rowEnd - $rowStart);
    ...
    {
        $rowStart = mb_strrpos($xml, '<w:tr ', ((strlen($xml) - $offset) * -1));
