...
class Pdf extends Container
{
    ...
    public function __construct($document, $config = [])
    {
        ...
        elseif (mb_strpos($document, 'DOCTYPE') !== FALSE) {
            ...
            throw new RuntimeException('Unrecognised document type! (' . $document . ')');
        }
    ...
    public function searchForBlock(string $blockname): array
    {
        $matches = parent::searchForBlock($this->documentXML, $blockname);
    ...
    {
        $string = $this->backend->getDocumentXML() instanceof \SimpleXMLElement
            ? $this->backend->getDocumentXML()->asXml()
            : '';
        foreach ($this->backend->getHeaderXMLs() as $part) {
            $string .= $part instanceof \SimpleXMLElement
                ? $part->asXml()
                : '';
        }
        foreach ($this->backend->getFooterXMLs() as $part) {
            $string .= $part instanceof \SimpleXMLElement
                ? $part->asXml()
                : '';
        }
        // Finds all ${dummy} fields
        preg_match_all('/\$\{(.*?)\}/mu', $string, $matches);
        ...
        foreach ($values as $key => $value) {
            if (in_array($key, $matches) === TRUE) {
                $translate[$this->normaliseStartTag($key)] =
                    str_replace(
                        "\n",
                        '</w:t><w:br/><w:t>',
                        htmlspecialchars(mb_convert_encoding($value, 'UTF-8'))
                    );
            }
            unset($values[$key]);
        }

        $this->backend->setValues($translate);
    }
     ...
     * $this
     */
    public function setValue(string $search, string $replace = \NULL, int $limit = -1)
    {
        $this->__call('setValue', [
            $search,
            $replace,
            $limit]);
